...
```{r ST_models}
prelaw_ST <- prelaw[prelaw$state_code == 37,]

prelaw_ST_model <- lm(formula=(subprime ~ as_of_year), data=prelaw_ST)
summary(prelaw_ST_model)

postlaw_ST <- postlaw[postlaw$state_code == 37,]

postlaw_ST_model <- lm(formula=(subprime ~ as_of_year), data=postlaw_ST)
summary(postlaw_ST_model)
...
ggplot(data=prelaw_ST, aes(x=as_of_year, y=subprime)) + geom_point(color='darkred') + 
  geom_smooth(method=lm, se=FALSE, color='darkgray') + geom_text(x=1992, y=150000, label=lm_eqn(prelaw_ST_model), parse=TRUE) + theme_minimal() + ggtitle('North Carolina Pre-Law')

ggplot(data=postlaw_ST, aes(x=as_of_year, y=subprime)) + geom_point(color='darkblue') + 
  geom_smooth(method=lm, se=FALSE, color='darkgray') + geom_text(x=2001, y=200000, label=lm_eqn(postlaw_ST_model), parse=TRUE) + theme_minimal() + ggtitle('North Carolina Post-Law')
```
