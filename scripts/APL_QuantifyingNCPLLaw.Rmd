...
state_data_2 <- read.csv('state_data_2.csv')
...
county_data_2 <- read.csv('county_data_2.csv')
...
```{r NC_models}
prelaw_NC <- prelaw[prelaw$state_code == 37,]

prelaw_NC_model <- lm(formula=(subprime ~ as_of_year), data=prelaw_NC)
summary(prelaw_NC_model)

postlaw_NC <- postlaw[postlaw$state_code == 37,]

postlaw_NC_model <- lm(formula=(subprime ~ as_of_year), data=postlaw_NC)
summary(postlaw_NC_model)
...
ggplot(data=prelaw_NC, aes(x=as_of_year, y=subprime)) + geom_point(color='darkred') + 
  geom_smooth(method=lm, se=FALSE, color='darkgray') + geom_text(x=1992, y=150000, label=lm_eqn(prelaw_NC_model), parse=TRUE) + theme_minimal()

ggplot(data=postlaw_NC, aes(x=as_of_year, y=subprime)) + geom_point(color='darkblue') + 
  geom_smooth(method=lm, se=FALSE, color='darkgray') + geom_text(x=2001, y=200000, label=lm_eqn(postlaw_NC_model), parse=TRUE) + theme_minimal()
```
